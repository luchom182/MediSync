...
export const AuthPage = () => {
  ...
  // Client ID de Google (Obtenido desde variables de entorno o predeterminado para pruebas)
  ...
  // Inicialización de Google Identity Services SDK (Popup real de Google)
  useEffect(() => {
      ...
      try {
        window.google.accounts.id.initialize({
          ...
          callback: async (response) => {
            setLoading(true);
            try {
              // response.credential es el token ID firmado oficialmente por Google
        ...
        // Renderizar el botón oficial de Google en la div #googleButtonContainer
  ...
  // Manejador alternativo del botón de Google
  const handleGoogleSignIn = async () => {
    setError(null);

    // Si Google SDK está cargado y tenemos Client ID, lanzar la ventana emergente real de Google
    if (window.google && googleClientId) {
      window.google.accounts.id.prompt((notification) => {
        if (notification.isNotDisplayed() || notification.isSkippedMoment()) {
          // Si el One-Tap fue ignorado, intentar simulación dev
          triggerDevGoogleAuth();
        }
      });
    } else {
      // Si aún no se ha configurado el Client ID real en .env, realizar autenticación de prueba
      triggerDevGoogleAuth();
    }
  };

  const triggerDevGoogleAuth = async () => {
    setLoading(true);
    try {
      const mockGoogleUser = {
        email: `paciente.gmail_${Math.floor(Math.random() * 1000)}@gmail.com`,
        nombre: 'Usuario Gmail (Dev Mode)',
  ...
  return (
    <div style={{
      ...
      <div className="glass-card" style={{ width: '100%', maxWidth: '400px', padding: '28px 24px', borderRadius: 'var(--radius-lg)' }}>
        ...
        {/* Contenedor del Botón Oficial de Google o Botón Personalizado */}
